...
namespace PhasmophobiaMenuExternal
{
    public class Offsets
    {
        ...
        public static void UpdateOffsets()
        {
            ...
            {
                GhostController = Program.SimpleMemoryReading.PrivateMemoryRegions.AsParallel().Where(r => r.Protect == Imports.MemoryProtect.ReadWrite && r.AllocationProtect == Imports.MemoryProtect.ReadWrite && r.State == Imports.MemoryState.Commit).SelectMany(r => Program.SimpleMemoryReading.AOBScanRegion(r, "E0 57 09 ?? ?? ?? ?? 00 00 00 00 00 00 00 00 00 ?? ?? ?? ??")).Where(a =>
                {
                ...
                CursedItemsController = Program.SimpleMemoryReading.PrivateMemoryRegions.AsParallel().Where(r => r.Protect == Imports.MemoryProtect.ReadWrite && r.AllocationProtect == Imports.MemoryProtect.ReadWrite && r.State == Imports.MemoryState.Commit).SelectMany(r => Program.SimpleMemoryReading.AOBScanRegion(r, "A0 08 60 ?? ?? ?? ?? 00 00 00 00 00 00 00 00 00 ?? ?? ?? ?? ?? ?? ?? ?? 00 00 00 00 00 00 00 00 00 00 00 00")).Where(a =>
                {
                    return new[]
                    {
                ...
                MapController = Program.SimpleMemoryReading.PrivateMemoryRegions.AsParallel().Where(r => r.Protect == Imports.MemoryProtect.ReadWrite && r.AllocationProtect == Imports.MemoryProtect.ReadWrite && r.State == Imports.MemoryState.Commit).SelectMany(r => Program.SimpleMemoryReading.AOBScanRegion(r, "B0 EF F7 ?? ?? ?? ?? 00 00 00 00 00 00 00 00 ?? ?? ?? ?? ?? ?? ?? ?? 00 00 00 00 00 00 00 00 ?? ?? ?? ??")).Where(a =>
                {
                ...
                FOV = Program.SimpleMemoryReading.PrivateMemoryRegions.AsParallel().Where(r => r.Protect == Imports.MemoryProtect.ReadWrite && r.AllocationProtect == Imports.MemoryProtect.NoAccess && r.State == Imports.MemoryState.Commit).SelectMany(r => Program.SimpleMemoryReading.AOBScanRegion(r, "00 00 B4 42 00 00 00 00 00 00 80 3F ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ??")).Where(a =>
                {
                ...
                // needs redone at a later date very unreliable
                /*
                PlayerX = Program.SimpleMemoryReading.PrivateMemoryRegions.AsParallel().SelectMany(r => Program.SimpleMemoryReading.AOBScanRegion(r, "79 88 76 41 6B 40 B9 3F ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ??")).Where(a =>
                {
